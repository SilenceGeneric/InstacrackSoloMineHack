...
# Prompt user for Instagram username input
printf "${green}\nEnter the Instagram username to simulate cracking: ${reset}"
read -p "> " username

# Validate the input (ensure the username is not empty)
if [ -z "$username" ]; then
    echo "${red}Error: No username entered. Please provide a valid username.${reset}"
    exit 1
fi

# Simulate Instagram password cracking (no real interaction with Instagram)
